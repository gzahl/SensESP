...
void setup_fuel_flow_meter(int inflow_pin, int return_flow_pin) {
  ...
  diff->connect_from(freqIn, freqOut)
      ...
      ->connect_to(new MovingAverage<float, float>(
          10, 1.,
          "/fuelflow/fuel/average/calibrate"))  // this is the same as above,
                                                // but averaged over 10 s
      ->connect_to(new SKOutputNumber("propulsion.main.fuel.averageRate",
                                      "/fuelflow/fuel/average/sk"));
...
void setup_rpm_meter(SensESPApp* seapp, int input_pin) {
  ...
  (new DigitalInputCounter(input_pin, INPUT_PULLUP, RISING, 500))
      ->connect_to<float>(
          new Frequency(1. / 97., "/sensors/engine_rpm/calibrate"))
